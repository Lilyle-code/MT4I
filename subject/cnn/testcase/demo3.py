...
def testAPI(teststr,trainstr):
    ...
    #def norm(pool1,lsize=5):#变体28-CRP-97
        return tf.nn.lrn(pool1,lsize,bias=1.0,alpha=0.001*9.0,beta=0.75)#变体3-10
    ...
    with tf.Session() as sess:
        ...
        for i in range(0,300):
            if a[i]!=b[i]:
                c=c+1
        d=c/300'''
        print(sess.run(pred,feed_dict={x: img1, y: label1, keep_prob: dropout}))
        return sess.run(tf.argmax(pred,1),feed_dict={x: img1, y: label1, keep_prob: dropout})
#result=testAPI("E:/Program Files/PyCharm Community Edition 2020.1.1/projects/CNN/show_image/test","E:/Program Files/PyCharm Community Edition 2020.1.1/projects/CNN/show_image/train")
#print(result)