...
def testAPI(teststr,trainstr):
    ...
    weights={
        ...
        'out':tf.Variable(tf.random_normal([4096,n_classes],stddev=0.01,seed=2))#变体59-172
    ...
    with tf.Session() as sess:
        ...
        for i in range(0,300):
            if a[i]!=b[i]:
                c=c+1
        d=c/300'''
        final = sess.run(tf.argmax(pred, 1), feed_dict={x: img1, y: label1, keep_prob: dropout})
        del sess
        del weights
        del biases
        del pred
